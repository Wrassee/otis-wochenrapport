...
Futtasd mind a 7 fájlt **növekvő sorrendben**:

```text
001_init.sql                    ← alapséma (profilok, helyek, időkódok, időbejegyzések)
002_favorites.sql               ← kedvenc/utoljára használt liftek
...
| 002 | `002_favorites.sql` | `user_favorites` tábla (kedvenc liftek, `UNIQUE(user_id, anlagenummer)`) | ✅ | ✅ — mindig a 001 után, de a 003–007-hez képest szabad sorrendben |
...
A 003/004 és a 006/007 **ugyanazt a célt** szolgálják (a tábla bekerül a `supabase_realtime` publikációba + `REPLICA IDENTITY FULL`), csak **két különböző időpontban**:

| Mikor fut a realtime-bekötés? | Fájl |
|---|---|
| **Tábla létrehozásakor** (ajánlott út) | `003` és `004` |
| **Utólag, biztosítékként** (régi DB-kre) | `006` és `007` |
...
| A realtime nem érkezik meg az appba | A tábla nincs benne a `supabase_realtime` publikációban | Futtasd le a `006`/`007`-et (idempotens) vagy a frissített `003`/`004`-et |
...
- A `006`/`007` mintája (guard-elt `DO` blokk + `EXECUTE`-os `REPLICA IDENTITY`) követendő, ha egy meglévő táblához adsz realtime-ot utólag.
