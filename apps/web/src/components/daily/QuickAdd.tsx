...
import { Clock, Plus, Zap, ChevronDown, ChevronUp } from 'lucide-react'
  ...
  return (
    <Card>
      ...
      <button
        ...
        className="w-full flex items-center gap-2.5 transition-all active:scale-[0.99]"
      >
        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center shadow-lg shadow-amber-500/20 flex-shrink-0">
        ...
        <div className="space-y-2">
          {eligibleEntries
            ...
            .map((entry) => (
              <div
                key={entry.id}
                className="flex items-center justify-between p-3.5 bg-otis-50/50 dark:bg-white/3 rounded-2xl border border-otis-200/20 dark:border-white/5 group hover:border-otis-300/30 transition-all duration-200"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-sm text-otis-800 dark:text-white">
                      {entry.location_anlagenummer || '—'}
                    </span>
                    {entry.activity_code && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-otis-100/50 dark:bg-otis-800/50 text-otis-600 dark:text-otis-300 font-medium">
                        {entry.activity_code}
                      </span>
                    )}
                  </div>
